...
func NewRouterWithSchemaStatus(cfg *config.Config, db *gorm.DB, schemaOK *bool) *gin.Engine {
	...
	api := r.Group("/api/v1")
	{
		...
		}))
		{
			...
			caseH := handlers.NewCaseHandler(nil)
			auth.GET("/cases/:id", caseH.UserCaseStatus)
			auth.POST("/admin/cases", middleware.BodyLimit(middleware.MaxBodyDefault), caseH.CreateCase)
			auth.GET("/admin/cases", caseH.ListCases)
			auth.GET("/admin/cases/:id", caseH.GetCase)
			auth.POST("/admin/cases/:id/assign", middleware.BodyLimit(middleware.MaxBodyDefault), caseH.AssignCase)
			auth.POST("/admin/cases/:id/transition", middleware.BodyLimit(middleware.MaxBodyDefault), caseH.TransitionCase)
			auth.POST("/admin/cases/:id/notes", middleware.BodyLimit(middleware.MaxBodyDefault), caseH.AddNote)
			auth.GET("/admin/cases/:id/audits", caseH.ListAudits)
			...
			auth.POST("/liveops/instances/:id/claim", middleware.BodyLimit(middleware.MaxBodyDefault), liveopsH.Claim)
			auth.POST("/liveops/instances/:id/compensate", middleware.BodyLimit(middleware.MaxBodyDefault), liveopsH.Compensate)
			auth.PUT("/ops/liveops/definitions", middleware.BodyLimit(middleware.MaxBodyDefault), liveopsH.UpsertDefinition)
			auth.POST("/ops/liveops/instances/:id/cancel", middleware.BodyLimit(middleware.MaxBodyDefault), liveopsH.CancelInstance)
			auth.POST("/ops/liveops/instances/:id/settle", middleware.BodyLimit(middleware.MaxBodyDefault), liveopsH.SettleInstance)
			...
			if appDB != nil && deviceRepo != nil {
				...
				auth.GET("/narrative/nodes/:node_id", narrH.GetNode)
				auth.GET("/narrative/progress", narrH.GetProgress)
				auth.GET("/narrative/progress/all", narrH.PullAll)
				auth.POST("/narrative/choices", middleware.BodyLimit(middleware.MaxBodyDefault), narrH.SubmitChoice)
				auth.POST("/narrative/seen", middleware.BodyLimit(middleware.MaxBodyDefault), narrH.MarkSeen)
				auth.GET("/narrative/seen", narrH.ListSeen)
